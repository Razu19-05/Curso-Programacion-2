...
void operator<<(const char *file_name, struct Reviews &reviews) {
    ...
    int &n_reviews = reviews.n_reviews;//se pone & porque es un dato que va a variar
    while (true) {
        input >> review;
        if (input.eof())break;
        reviews.reviews[n_reviews] = review;//guarda cada review en
        n_reviews++;
    }
}
...
void operator+=(Reviews &reviews, int *estadisticas) {
    for(int i=0; i<reviews.n_reviews; i++){
        ...
        if(reviews.reviews[i].rating >= 0.6) estadisticas[2]++;
    }
}